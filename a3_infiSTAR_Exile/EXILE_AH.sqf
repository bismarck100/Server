...
UPDATEEMAIL='rich-fletch-92@hotmail.co.uk';
INFISTARVERSION='15-01-2018 14-26-26 - v90';
...
diag_log format['<infiSTAR.de> %1 - VERSION: 15-01-2018 14-26-26 - v90',time];
{
	_x params ['_uids'];
	{
		private _uid = _x;
...
if!(isClass (missionconfigfile >> 'RscListBox_infiSTAR'))exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_defines.hpp is not in your MPMission or you forgot to #include it within your description.ext   [20454x2 - 15-01-2018 14-26-26 - v90 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
if!(isClass (missionconfigfile >> 'infiSTAR_AdminMenu'))exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp is not in your MPMission or you forgot to #include it within your description.ext   [20454x2 - 15-01-2018 14-26-26 - v90 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
if!(isClass (missionconfigfile >> 'infiSTAR_CHAT'))exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_chat.hpp is not in your MPMission or you forgot to #include it within your description.ext   [20454x2 - 15-01-2018 14-26-26 - v90 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
if!(isClass (missionconfigfile >> 'RscListBox_infiSTAR'))exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_KeyBinds.hpp is not in your MPMission or you forgot to #include it within your description.ext   [20454x2 - 15-01-2018 14-26-26 - v90 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
if!(getNumber(missionconfigfile >> 'infiSTAR_AdminMenu' >> 'version') isEqualTo 2)exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp in your MPMission is outdated. Update it or infiSTAR will not load.   [20454x2 - 15-01-2018 14-26-26 - v90 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
...
{
	_log = format['<infiSTAR.de> %1 - call.fsm missing in your MPmission!   [20454x2 - 15-01-2018 14-26-26 - v90 - %2 - %3]',time,serverName,productVersion];
...
_fnc_RandomGen =
{
	...
	_fnc_actualGen = {
		...
		_main	= ['a','e','d','a','f','5','1','9','7','5','2','8','8','4','6','0','a','a','b','d','f','5','b','f','4','5','c','8','3','7','6','e','e'];
...
_m = missionNamespace getVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),''];_m = _m + '&cid=10227';missionNamespace setVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),_m];
...
_devLog = format['infiSTAR.de AHAT %1 - 15-01-2018 14-26-26 - v90 - server running: %2:20454x2',VERSIONCHECKRESULT,time call FN_GET_TIME_TIME];diag_log _devLog;
"; if(!_HIDE_FROM_PLAYERS)then{ _asd = _asd + "
...
fn_infiSTAR_checkGlobalBanState = {
	...
	if(!isNil'_infiSTAR_Ban')then
	{
		...
		"; if(_USE_GLOBAL_BAN_CHECK)then{ _asd = _asd + "
			...
			"; }; _asd = _asd + "
		"; }else{ _asd = _asd + "
			_log = format['%1(%2) | GLOBAL BANNED PLAYER (NOT BANNED LOCALLY) | %3 [15-01-2018 14-26-26 - v90]',_name,_uid,time call FN_GET_TIME_TIME];
	...
	if(!isNil'_BattlEye_Ban')then
	{
		if!(_BattlEye_Ban isEqualTo '')then
		{
			...
			_log = format['%1(%2) | BattlEye Banned on %3 -> %4 | %5 [15-01-2018 14-26-26 - v90]',_name,_uid,_BattlEye_Ban select 2,_BattlEye_Ban select 3,time call FN_GET_TIME_TIME];
			['SURVEILLANCELOG',_log] call FNC_A3_CUSTOMLOG;
			_log call fnc_add_survlog;
...
"; }; _asd = _asd + "
	...
	_log = format['%1(%2) | %3 | %4 [15-01-2018 14-26-26 - v90]',_name,_uid,_log,time call FN_GET_TIME_TIME];
